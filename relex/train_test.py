...
def segment (train, test, labels, no_rel=None):
    ...
    seg_test = Set_Connection(CSV=False, dataset=test, rel_labels=labels, test=True).data_object
    print("Performing segment TRAIN---------------------------------------------------------------------")
    if no_rel:
        seg_train = Set_Connection(CSV=False, dataset=train, rel_labels=labels, no_labels=no_rel).data_object
    else:
        seg_train = Set_Connection(CSV=False, dataset=train, rel_labels=labels).data_object
