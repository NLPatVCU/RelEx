...
if sys.argv[3] == 'segment':
    ...
    if sys.argv[3] == 'single':
        ...
        model = Model(data_object=connection_train, data_object_test=connection_test, segment=True, test=True, multilabel=True)
        embedding = Embeddings(embedding_path, model, embedding_dim=int(sys.argv[2]))
        sent_cnn = Sentence_CNN(model, embedding, filters=300, drop_out=0.1, filter_conv=3, optimizer='adam',
